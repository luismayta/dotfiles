...
- **AND** `CLEAN_BASE_DIR_PATTERNS` does not duplicate them

#### Scenario: All patterns sourced from configuration
- **WHEN** `_cleanup::unnecessary` executes
- **THEN** directory patterns come exclusively from `CLEAN_BASE_DIR_PATTERNS` and `CLEAN_AGGRESSIVE_PATTERNS`
