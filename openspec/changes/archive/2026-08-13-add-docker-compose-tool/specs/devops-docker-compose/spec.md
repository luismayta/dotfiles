...
The devops module SHALL expose docker-compose configuration through `DEVOPS_DOCKER_COMPOSE_*` environment variables (`PACKAGE_NAME`, `INSTALL_CMD`, `CONFIG_DIR`).

#### Scenario: Configuration variables are defined
- **WHEN** the devops module loads
- **THEN** `DEVOPS_DOCKER_COMPOSE_PACKAGE_NAME`, `DEVOPS_DOCKER_COMPOSE_INSTALL_CMD` and `DEVOPS_DOCKER_COMPOSE_CONFIG_DIR` SHALL be defined
...
The devops module SHALL install docker-compose on module load when it is missing, using the configured install command.

#### Scenario: auto-install on missing binary
- **WHEN** the devops module loads and docker-compose is not present
- **THEN** the install function SHALL run using `DEVOPS_DOCKER_COMPOSE_INSTALL_CMD`
