...
- **AND** `CLEAN_AGGRESSIVE_PATTERNS` is not set
- **THEN** system does not remove directories named `build`, `dist`, `out`, `release`, `debug`, `target`, `vendor`, `tmp`, `temp`, `coverage`, `eggs`, or `venv`
- **AND** the default `CLEAN_BASE_DIR_PATTERNS` excludes those names

#### Scenario: Generic names removed when opted in
- **WHEN** user sets `CLEAN_AGGRESSIVE_PATTERNS="build|dist|target"` before module load
- **AND** runs `cleanup`
...
- **AND** a pattern is present in both `CLEAN_BASE_DIR_PATTERNS` and `CLEAN_AGGRESSIVE_PATTERNS`
