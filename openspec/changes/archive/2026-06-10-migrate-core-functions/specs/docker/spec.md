...
The system SHALL provide `awscli()` that runs the AWS CLI inside a Docker container.
...
The system SHALL provide `aws-shell()` that runs the AWS Shell inside a Docker container with the same mount and env configuration.
...
The system SHALL provide `ytdl()` and `ytd-mp3()` functions that run youtube-dl inside a Docker container.
...
The system SHALL provide `nyancat()` that runs the nyan cat animation in Docker.
...
The system SHALL provide `komiser()` that runs the Komiser AWS dashboard in Docker.
...
### Requirement: Module structure follows dotfiles convention

The docker module SHALL be self-contained within `zsh/modules/docker/` with `plugin.zsh` as the entry point.

#### Scenario: Entry point exists
- **WHEN** the module directory is inspected
- **THEN** `zsh/modules/docker/plugin.zsh` exists and is executable

#### Scenario: Subdirectory structure is complete
- **WHEN** the module files are enumerated
- **THEN** `pkg/` subdirectory contains `main.zsh`, `base.zsh`, `osx.zsh`, `linux.zsh`, `alias.zsh`, `helper.zsh`