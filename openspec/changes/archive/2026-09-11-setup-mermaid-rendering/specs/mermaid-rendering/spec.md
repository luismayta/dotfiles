...
The system SHALL install chrome-headless-shell in a persistent location (`~/.local/share/chrome-headless-shell`) using `npx @puppeteer/browsers install chrome-headless-shell@stable --path`. The install SHALL be idempotent — if the binary already exists at the target path, the install SHALL be skipped.

#### Scenario: First-time install
- **WHEN** `~/.local/share/chrome-headless-shell` does not exist
- **THEN** the system SHALL execute `npx @puppeteer/browsers install chrome-headless-shell@stable --path ~/.local/share/chrome-headless-shell` and the binary SHALL be available at that path
