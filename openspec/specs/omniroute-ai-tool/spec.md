# omniroute-ai-tool Specification
...
- `ZSH_AI_OMNIROUTE_INSTALL_CMD` — bun install command (`bun add -g`)
...
The system SHALL provide `ai::internal::omniroute::load` that returns silently when the `omniroute` binary is not present, since the bun global bin directory is already on PATH.
...
The system SHALL provide `ai::internal::omniroute::install` to install OmniRoute globally via bun.
