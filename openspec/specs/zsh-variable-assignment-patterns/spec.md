...
Establece el patrón correcto para asignación de variables en módulos zsh: `:=` para valores por defecto que pueden ser sobrescritos, asignación directa para overrides obligatorios de plataforma.
...
### Requirement: Overrides de plataforma usan asignación directa
Los archivos OS-específicos (`osx.zsh`, `linux.zsh`) DEBEN usar asignación directa (`VAR="value"`) para sobreescribir valores por defecto cuando el override es obligatorio.

#### Scenario: macOS con binario diferente
- **WHEN** el módulo se carga en macOS y el binario tiene nombre diferente (ej: `hx` vs `helix`)
- **THEN** el archivo `osx.zsh` usa asignación directa para forzar el override
