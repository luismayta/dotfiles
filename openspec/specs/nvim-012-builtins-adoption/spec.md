# nvim-012-builtins-adoption

## Purpose

Adopt nvim 0.12 built-ins in place of third-party plugins where they fully replace them (inlay hints, undotree), keep plugins that lack a complete built-in replacement (diffview, noice, nvim-cmp), and simplify `options.lua` by removing options that are now nvim 0.12 defaults.
