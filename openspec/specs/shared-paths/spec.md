...
- **WHEN** `zsh/core/config/paths.zsh` is sourced
- **THEN** `DOTFILES_CORE_DIR`, `DOTFILES_MOD_DIR`, `DOTFILES_BACKUP_DIR`, `DOTFILES_CACHE_DIR`, `LOCAL_PATH_BIN`, `HOMEBREW_BIN_PATH`, `PRIVATERC`, and `CUSTOMRC` SHALL be exported as environment variables