...
```bash
git remote add upstream  https://github.com/hadenlabs/dotfiles.git
```

Now your **upstream** points to **hadenlabs/dotfiles**.
...
git fetch upstream # assuming upstream is rfid/dotfiles
...
All merge requests are automatically tested using [Github Actions](https://github.com/hadenlabs/dotfiles/actions). In case some tests fail, fix the issues or describe why the fix cannot be done.
