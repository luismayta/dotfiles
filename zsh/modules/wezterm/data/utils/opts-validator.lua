...
local function validate_opts_schema(schema)
  ...
  for _, opt in ipairs(schema) do
    ...
    assert(type(opt.required) == "boolean" or type(opt.required) == "nil", "required must be a boolean")
    assert(
      opt.type == "boolean" or opt.type == "number" or opt.type == "string" or opt.type == "table",
