...
function cleanup::system::trash {
    ...
    [[ -n "${ZSH_CLEAN_OSX_TRASH_VOLUMES}" ]] && _cleanup::safe_remove "${ZSH_CLEAN_OSX_TRASH_VOLUMES}"
    [[ -n "${ZSH_CLEAN_OSX_TRASH_MAIN}" ]] && _cleanup::safe_remove "${ZSH_CLEAN_OSX_TRASH_MAIN}"
...
function cleanup::system::logs {
    ...
    [[ -n "${ZSH_CLEAN_OSX_LOG_MAIL}" ]] && _cleanup::safe_remove "${ZSH_CLEAN_OSX_LOG_MAIL}"
    [[ -n "${ZSH_CLEAN_OSX_LOG_CORE_SIMULATOR}" ]] && _cleanup::safe_remove "${ZSH_CLEAN_OSX_LOG_CORE_SIMULATOR}"
    message_success "Clear System Log Files..."
}

function cleanup::osx::adobe_cache {
    message_info "Clear Adobe Cache Files..."
    [[ -n "${ZSH_CLEAN_OSX_ADOBE_CACHE}" ]] && _cleanup::safe_remove "${ZSH_CLEAN_OSX_ADOBE_CACHE}"
    message_success "Clear Adobe Cache Files..."
}

function cleanup::osx::ios_application {
    message_info "Cleanup iOS Applications..."
    [[ -n "${ZSH_CLEAN_OSX_IOS_APPLICATIONS}" ]] && _cleanup::safe_remove "${ZSH_CLEAN_OSX_IOS_APPLICATIONS}"
    message_success "Cleanup iOS Applications..."
}

function cleanup::osx::ios_device_backup {
    message_info "Remove iOS Device Backups..."
    [[ -n "${ZSH_CLEAN_OSX_IOS_DEVICE_BACKUP}" ]] && _cleanup::safe_remove "${ZSH_CLEAN_OSX_IOS_DEVICE_BACKUP}"
    message_success "Remove iOS Device Backups..."
}

function cleanup::osx::xcode {
    message_info "Cleanup XCode Derived Data and Archives..."
    [[ -n "${ZSH_CLEAN_OSX_XCODE_DERIVED_DATA}" ]] && _cleanup::safe_remove "${ZSH_CLEAN_OSX_XCODE_DERIVED_DATA}"
    [[ -n "${ZSH_CLEAN_OSX_XCODE_ARCHIVES}" ]] && _cleanup::safe_remove "${ZSH_CLEAN_OSX_XCODE_ARCHIVES}"
